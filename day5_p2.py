...
def main():
    ...
    ids = sorted([ticket.identity for ticket in tickets])
    max_id = max(ids)

    all_possible = set(range(max_id))
    missing = all_possible - set(ids)

    # Didn't bother coding it, 685 was the only candidate
    print(f"Here are the possibilities, figure it out: {missing}")
