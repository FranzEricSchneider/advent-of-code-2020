...
class Point:
    def __init__(self, x, y, z, w, active=False):
        self.position = numpy.array([x, y, z, w], dtype=int)
        self.active = active
...
def main():
    ...
    with open("day17.txt", "r") as fin:
        for line in fin.readlines():
            for column, char in enumerate(line.strip()):
                points.append(Point(x=row, y=column, z=0, w=0, active=char == ACTIVE))
            row += 1
    ...
    for i in range(6):
        ...
        grid.step()
        print(f"Calculating round {i+1}/6")
